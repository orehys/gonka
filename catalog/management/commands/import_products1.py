...
from catalog.models import Category, Product, ProductImage, Attribute, ProductAttributeValue
...
SUBCATEGORY_MAP = {
    ...
    "diski": ("Диски", "wheels"),
    "prostvaki": ("Проставки", "wheels"),  # если в данных так написано; иначе исправь на "prostavi"/"prostavi"
...
def ensure_cat_by_subslug(sub_slug: str, Category):
    """Создать/получить подкатегорию по её слагу из SUBCATEGORY_MAP.
       Вернёт объект подкатегории (child)."""
    info = SUBCATEGORY_MAP.get(sub_slug)
    if not info:
        # фолбэк: сделаем название из слага
    ...
    # если имя у родителя пустое/английское — перезапишем русским
...
def ensure_category_path(path_text):
    ...
    for name in parts:
        ...
        if cat.parent_id != (parent.id if parent else None):
            cat.parent = parent; cat.name = name; cat.save(update_fields=["parent","name"])
        parent = cat
    return parent
...
def parse_specs(text):
    ...
    for part in re.split(r"[;\n]+", s):
        ...
        elif "-" in part and not re.match(r"^-?\d+(\.\d+)?$", part): k, v = part.split("-", 1)
        else: k, v = "Характеристика", part
        pairs.append((k.strip(), v.strip()))
    return [(k, v) for k, v in pairs if k and v]

def split_multi_values(key, val):
    """Дробим значения для нужных ключей на отдельные элементы."""
...
def normalize_folder_under(root: Path, folder_text: str) -> Path | None:
    ...
    p = Path(raw)
    if p.is_absolute():  # если в xlsx абсолютный путь
        return p
...
class Command(BaseCommand):
    help = "Импорт из Excel: Название, Категория, Артикул, Цена, Описание, Характеристики, Ссылка, Фото, Папка"
    ...
    def handle(self, *args, **opts):
        xlsx = Path(opts["xlsx"])
        if not xlsx.exists(): raise CommandError(f"Excel не найден: {xlsx}")
        df = pd.read_excel(xlsx, sheet_name=opts.get("sheet") or 0)
        if opts.get("limit"): df = df.head(int(opts["limit"]))
        ...
        for must, nm in [(COL_TITLE,"Название"), (COL_SKU,"Артикул")]:
            if not must: raise CommandError(f"Нет обязательной колонки: {nm}")

        created_p = updated_p = added_imgs = 0
        ...
        for _, row in df.iterrows():
            ...
            if opts.get("only_specs"):
                try:
                    ...
                    continue  # пропустим строки без существующего товара
            else:
                ...
                if created:
                    ...
                    if changed:
                        product.save(); updated_p += 1

                # ФОТО
                if opts["replace-images"] and product.images.exists():
            ...
            # ХАРАКТЕРИСТИКИ — выполняем в ОБОИХ режимах
            ...
            for k, v in specs:
                ...
                for one in split_multi_values(key, val):
                    ProductAttributeValue.objects.get_or_create(product=product, attribute=attr, value=one)
